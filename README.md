...
#
Il faut maîtriser le modèle conceptuel des données (MCD), le modèle relationnel, le SQL (syntaxe, requêtes) pour travailler sur les SGBDR.
#
...
#
✅ Via des requêtes optimisées (SELECT, JOIN, WHERE, GROUP BY, SUM), il interroge la base données ; le moteur SQL détermine comment l’obtenir le plus rapidement possible. 
#
...
#
✅ Envoie les requêtes SQL.
#
✅ Récupère les résultats bruts.
#
✅ Applique la logique métier complexe (calculs spécifiques, enchaînements conditionnels) qu’il serait ardu ou lent d’écrire en SQL pur.
#
